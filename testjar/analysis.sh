...
# Strange Error!
EXTRA_ARG="${PLATFORM} ${OPEN_PROGRAM} ${SOUFFLE_MODE} ${SOUFFLE_JOBS} ${CFG} ${JIMPLE} ${EXTRA_LOGIC} ${INFORMATION_FLOW} ${LOG}"

cd $DOOP_HOME

CMD="${DOOP_HOME}/doop -a $ANALYSIS -i ${INPUT} -app-only --id ${ID} ${EXTRA_ARG}"
echo "doop: $CMD"
eval "${DOOP_HOME}/doop -a $ANALYSIS -i ${INPUT} -app-only --id ${ID} ${EXTRA_ARG}"
